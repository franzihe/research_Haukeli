...
def plot_skewT_MEPS(T, Td, z, p, u, v, hour, text):
    fig = plt.figure(1, figsize=(16, 20))#, edgecolor = 'k')
    gs = GridSpec(1,12)
# sounding
    ax1 = plt.subplot(gs[0,0:11])
...
# plot Temperature
    ...
    for ens_memb in range(1,10):
        if len(T[ens_memb]) == 0:
            ...
            ax1.semilogy(T[hour,:] + 
                     skewt.skew(p[hour,:]), p[hour,:], basey=math.e, 
                     color =memb_col_T, linewidth = (linewidth_T))
  ...
  #          continue
   #     else:
    #        ax1.semilogy( Td[ens_memb][hour,:] + 
     #            skewt.skew(p[ens_memb][hour,:]), p[ens_memb][hour,:], basey=math.e, 
      #           color=memb_col_Td, linewidth = (linewidth_Td))
    ax1.semilogy(T[hour,:] + skewt.skew(p[hour,:]),p[hour,:], basey=math.e, color =linecolor_T, linewidth = (linewidth_T+1.5))

    ax1.semilogy(Td[hour,:] + skewt.skew(p[hour,:]), p[hour,:], basey=math.e, color=linecolor_Td, linewidth = (linewidth_Td+1.5))

# wind barbs
    ax4 = plt.subplot(gs[0,-1])
    skewt.plot_wind_axes(ax4)
    skewt.plot_wind_barbs(ax4,z[hour,:],p[hour,:],u[hour,:],v[hour,:])
...
#    plevs_std = [100000,85000,70000,50000,40000,30000,25000,20000,15000]
    plevs_std = skewt.plevs_std
    for plvl in plevs_std:
        zlvl = interp.interp_height(z[hour,:],p[hour,:],plvl)
        skewt.label_m(Tmin+2.55,plvl, str(int(zlvl)), ax1)
...
# plot wind barbs on left side of plot.  move this?  right side?
    ...
    if (u is not None and v is not None):
      #draw_wind_line(axes)
        for i in np.arange(0,len(z[hour,:]),2):
            if (p[hour,:][i] > pt_plot):
                plt.barbs(Tmin+4,p[hour,:][i],u[hour,:][i],v[hour,:][i], length=8, linewidth=2.)
